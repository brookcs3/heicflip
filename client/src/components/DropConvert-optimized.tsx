...
function DropConvertInner() {
    ...
    if (capabilities.hasWebWorker) {
      try {
        ...
        worker.onmessage = (event) => {
          // Get current jpgToHeic state for debugging
          const currentJpgToAvifState = jpgToHeic;
          console.log('Worker message received:', event.data, 'jpgToHeic setting:', currentJpgToAvifState);
            ...
            setTimeout(() => {
              if (result instanceof Blob) {
                ...
                if (actualFileCount === 1) {
                  ...
                  const fileExtension = extension || (jpgToHeic ? '.heic' : '.jpg');
  ...
  }, [capabilities.hasWebWorker, jpgToHeic]); // Add jpgToHeic to the dependency array so the worker updates when it changes
  ...
  const convertFiles = async () => {
    ...
    try {
      ...
      console.log('Starting conversion with settings:', {
        ...
        conversionMode: jpgToHeic ? 'JPG to HEIC' : 'HEIC to JPG'
      });
      ...
      if (workerRef.current) {
        ...
        workerRef.current.postMessage({
          type: processingType,
          files,
          jpgToHeic, // Include conversion mode
      ...
      if (totalFiles === 1) {
        ...
        const blob = new Blob([fileData], { 
          type: jpgToHeic ? 'image/heic' : 'image/jpeg' 
        });
        ...
        setTimeout(() => {
          ...
          const fileName = safeFileName.replace(/\.(heic|png|jpe?g)$/i, jpgToHeic ? '.heic' : '.jpg');
          downloadLink.download = fileName;
          downloadLink.setAttribute('download', fileName); // Explicit download attribute
        ...
        try {
          ...
          for (let i = 0; i < totalFiles; i++) {
            ...
            const outputName = file.name.replace(/\.(heic|png|jpe?g)$/i, jpgToHeic ? '.heic' : '.jpg');
  ...
  const toggleConversionMode = useCallback(() => {
    setJpgToHeic((prev: boolean) => !prev);
  }, []);
    ...
    <div className="w-full max-w-3xl mx-auto bg-background/50 backdrop-blur-sm rounded-xl border shadow-sm p-5 mb-8">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <span className={`text-sm ${!jpgToHeic ? 'font-semibold text-primary' : 'text-muted-foreground'}`}>
            HEIC to JPG
          </span>
          <Switch 
            checked={jpgToHeic} 
            onCheckedChange={toggleConversionMode} 
            className={jpgToHeic ? 'bg-primary' : ''}
          />
          <span className={`text-sm ${jpgToHeic ? 'font-semibold text-primary' : 'text-muted-foreground'}`}>
            JPG to HEIC
          </span>
      ...
      {status === 'idle' && (
        ...
        >
          ...
          <p className="text-sm text-muted-foreground mb-3">
            {jpgToHeic
              ? 'Convert JPG/PNG to space-saving HEIC format' 
        ...
        <div className="rounded-lg border p-4">
          <div className="mb-4">
            ...
            <p className="text-sm text-muted-foreground">
              {jpgToHeic
                ? 'Convert to space-saving HEIC format'
              ...
              onClick={() => {
                ...
                if (downloadUrl) {
                  ...
                  link.download = files.length === 1 
                    ? files[0].name.replace(/\.(heic|png|jpe?g)$/i, jpgToHeic ? '.heic' : '.jpg')
