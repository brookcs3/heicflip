...
function DropConvertInner() {
  ...
  const onDrop = useCallback((acceptedFiles: File[]) => {
    ...
    if (acceptedImageFiles.length === 0) {
      setStatus('error');
      setErrorMessage('Please select image files (HEIC, PNG, JPG)');
      return;
    }
              ...
              if (result instanceof Blob) {
                ...
                if (actualFileCount === 1) {
                  ...
                  let baseFileName = originalFileName || 
                                    (files[0] ? files[0].name.replace(/\.(heic|png|jpe?g)$/i, '') : 'converted-file');
                  
                  // Then add the proper extension based on what we received from the worker
                  const fileExtension = extension || (jpgToAvif ? '.heic' : '.jpg');
  ...
  const convertFiles = async () => {
    ...
    try {
      ...
      console.log('Starting conversion with settings:', {
        ...
        conversionMode: jpgToAvif ? 'JPG to HEIC' : 'HEIC to JPG'
      });
      ...
      if (totalFiles === 1) {
        ...
        const blob = new Blob([fileData], { 
          type: jpgToAvif ? 'image/heic' : 'image/jpeg' 
        });
        ...
        setTimeout(() => {
          ...
          const fileName = safeFileName.replace(/\.(heic|png|jpe?g)$/i, jpgToAvif ? '.heic' : '.jpg');
          downloadLink.download = fileName;
          downloadLink.setAttribute('download', fileName); // Explicit download attribute
        ...
        try {
          ...
          for (let i = 0; i < totalFiles; i++) {
            ...
            const outputName = file.name.replace(/\.(heic|png|jpe?g)$/i, jpgToAvif ? '.heic' : '.jpg');
  ...
  // Toggle between JPG-to-HEIC and HEIC-to-JPG modes
  ...
  return (
    <div className="w-full max-w-3xl mx-auto bg-white backdrop-blur-sm rounded-xl border shadow-blue-lg p-6 mb-8 card-hover">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2 p-2 bg-amber-50 rounded-lg shadow-blue">
          ...
          <ArrowLeftRight className="ml-1.5 w-4 h-4 text-amber-500" />
        </div>
      </div>
      ...
      {status === 'idle' && (
        <div 
          ...
          className={`enhanced-dropzone rounded-lg p-10 text-center cursor-pointer transition-all ${
            isDragActive 
              ? 'active border-amber-500 bg-amber-50/70' 
        ...
        >
          ...
          <p className="text-sm text-muted-foreground mb-3">
            {jpgToAvif
              ? 'Convert JPG/PNG to compact HEIC format' 
      ...
      {status === 'ready' && (
        <div className="rounded-lg border shadow-blue p-5 bg-gradient-to-br from-white to-amber-50/50">
          <div className="mb-4">
            <h3 className="text-lg font-bold gradient-text mb-2">Ready to Convert {files.length} {files.length === 1 ? 'File' : 'Files'}</h3>
            <p className="text-sm text-muted-foreground">
              {jpgToAvif
                ? 'Convert to compact HEIC format'
          ...
          <div className="mb-4 max-h-40 overflow-y-auto rounded-md bg-white/80 p-3 shadow-sm">
            <ul className="space-y-1.5">
              {files.map((file, index) => (
                <li key={index} className="text-sm flex items-center text-muted-foreground hover:text-amber-600 transition-colors">
                  <FileImage className="mr-2 h-3.5 w-3.5 text-amber-500" />
                  {file.name} <span className="ml-1 text-xs text-amber-500">({formatFileSize(file.size)})</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="flex gap-3">
            <Button
              ...
              className="bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white shadow-blue"
            >
            ...
            <Button
              ...
              className="border-amber-200 hover:bg-amber-50 shadow-sm"
            >
      ...
      {status === 'processing' && (
        <div className="rounded-lg border shadow-blue-lg p-8 text-center bg-white">
          <div className="w-16 h-16 mx-auto mb-6 rounded-full bg-amber-100 flex items-center justify-center shadow-blue">
          ...
          <Progress value={progress} className="h-3 mb-3 bg-amber-100" style={{
            background: 'linear-gradient(to right, #ffedd5, #fef3c7)'
          }} />
          <p className="text-sm font-medium text-amber-500">{progress}% complete</p>
        </div>
      )}
      
      {status === 'success' && (
        <div className="rounded-lg border shadow-blue-lg p-8 text-center bg-gradient-to-br from-white to-amber-50">
          <div className="w-16 h-16 mx-auto mb-6 rounded-full bg-gradient-to-br from-green-400 to-amber-500 flex items-center justify-center shadow-blue">
          ...
          <div className="flex gap-3 justify-center">
            <Button
              onClick={() => {
                ...
                if (downloadUrl) {
                  ...
                  link.download = files.length === 1 
                    ? files[0].name.replace(/\.(heic|png|jpe?g)$/i, jpgToAvif ? '.heic' : '.jpg')
              ...
              className="bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white shadow-blue px-5 py-2"
            >
