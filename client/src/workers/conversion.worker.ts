...
self.onmessage = async (event) => {
  const { files, type, jpgToHeic, totalFiles, isSingleFile } = event.data;
  const fileCount = totalFiles || files.length;
  
  // Reduce logging for production performance
  // console.log('Worker received message:', { type, jpgToHeic, fileCount, isSingleFile, filesArrayLength: files.length });
  
  try {
    // Set up the correct MIME type and extension based on conversion direction
    const outputMimeType = jpgToHeic ? 'image/heic' : 'image/jpeg';
    const fileExtensionRegex = /\.(heic|png|jpe?g)$/i;
    const outputExtension = jpgToHeic ? '.heic' : '.jpg';
